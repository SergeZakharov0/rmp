...
PitchShifter::PitchShifter(int length)
{
	numSamples = length;
}
...
{
    int i = 0;
    for(float fidx = 0.0f; fidx < numSamples; fidx += factor, ++i)
    {
        int idx = (int)(fidx);
...
void PitchShifter::applyShifter(std::shared_ptr<AudioBuffer<float> > Input, std::shared_ptr<AudioBuffer<float> > Output[128][128], 
                                                                        int lowNote, int highNote, int lowVel, int highVel)
{
    ...
    //shiftTone(in_buff_ptr[0], out_buff_ptr[0],  12);
   // shiftTone(in_buff_ptr[1], out_buff_ptr[1],  12);
    for(int vel = lowVel; vel <= highVel; vel++)
    {
        int i = 1;
        for(int note = lowNote; note <= highNote; note++)
        {
            ...
            AudioBuffer<float> *out_buffer =  Output[note][vel].get();//(2, numSamples);// = (new AudioBuffer<float> (2, numSamples) );
       
            float *out_buff_ptr[2] = { out_buffer->getWritePointer(0), out_buffer->getWritePointer(1) };

            shiftTone(in_buff_ptr[0], out_buff_ptr[0],  i++);
            memcpy(out_buff_ptr[1], out_buff_ptr[0], sizeof(float)*numSamples);
